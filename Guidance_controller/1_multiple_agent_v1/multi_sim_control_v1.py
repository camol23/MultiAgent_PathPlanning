...
ploting.plot_scene(pso_routes.output_list, data_lists, obst_original, target_routes, cm_flag=True, obs_unknowns=obst_list_unkowns_converted)
...
#     'cm_flag': False
# }
# ploter = ploting.scene_plotter()
# ploter.initialize(plot_params)
...
# WORKS (but it's replaced by plot_scene)
# fig = plt.figure() 
# ax = fig.add_subplot(1, 1, 1) 


# for i, policy_i in enumerate(policy_list):

#     state_i = state_storage[:, i, :]
#     state_x = [x[0] for x in state_i]
#     state_y = [x[1] for x in state_i]

#     trajectory_x = [val[0] for val in policy_i.Tr]
#     trajectory_y = [val[1] for val in policy_i.Tr]
...
ploting.plot_scene(pso_routes.output_list, data_lists, obst_original, target_routes, cm_flag=True, states=state_storage, obs_unknowns=obst_list_unkowns_converted)

scene_axes, state_lines, figure, axes = ploting.animate_scene(pso_routes.output_list, data_lists, obst_original, target_routes, cm_flag=True, states=state_storage, obs_unknowns=obst_list_unkowns_converted)

jump = 50
def animate_update(frame):
    
    # line2.set_ydata(z2[:frame])    
    # for i, state_ax in enumerate(state_lines):
    #     state_lines[i] = state_i.set_data(state_storage[:frame, i, 0], state_storage[:frame, i, 1])
    state_lines_2 = []
    # print(state_storage[:frame, 0, 0].shape)    
    for i in range(0, num_agents):

        

        if brake_actived[frame*jump, i, 0] :
            # axes.add_patch(Circle((state_storage[frame*jump, 0, 0], state_storage[frame*jump, 0, 1]), 5, alpha=0.1))
            color_agent = 'black'
