...
def detect_obs_sim(state, obs_list, detect_obs_params, momentary_signal=0):
    ...
    for i in range(0, len(obs_list)):
        ...
        if num_collisitons > 0 :
            collision_flag = 1   
            sensor_dist = compute_sensor_distance(rays_x, rays_y, state, obs_list[i])         
            break
        else:
            collision_flag = 0
...
def detect_obs_sim_v2(state, obs_list, detect_obs_params, momentary_signal=0):
    '''
        Funtunction to Dectect the obstacle in a 
        certain range during simulation        
    ...
    sensor_dist = 1000 # Out of the range
    obst_id = -1
    for i in range(0, len(obs_list)):
        #  y_botton < X > y_up  (X.shape( num_points ))
        # mask_collision = (obs_list[i][1] < rays_y) & (rays_y < (obs_list[i][3]) )                     # last is a coordinate 
    ...
    return collision_flag, sensor_dist, obst_id

def compute_sensor_distance(ray_x, ray_y, state, obst):
    '''
        a _ d
        |   |
        b _ c

        the collision is detected for a point inside of the obstacle, then
        the ray only can collide with one Obstacle side
    '''
    
    # Sensor line
    m_array = (ray_y - state[1])/(ray_x - state[0])
    b_array = (ray_y) - ray_x*m_array

    # segment ab (x1 = CTE)
    y_ab = obst[0]*m_array + b_array
    mask_ab = (obst[1] <= y_ab) & (y_ab <= (obst[1] + obst[3]) )

    output = 1000
    if np.sum(mask_ab) > 0 :
        mask_ab_val = np.logical_not(mask_ab)*1000
        y_collied = mask_ab_val*y_ab + y_ab
        dist = np.sqrt( (obst[0]-state[0])**2 + (y_collied-state[1])**2)
        
        output = np.min(dist)

    # segment dc (x2 = CTE)
    obst_x2 = (obst[0] + obst[2])
    y_dc = obst_x2*m_array + b_array
    mask_dc = (obst[1] <= y_ab) & (y_ab <= (obst[1] + obst[3]) )

    if np.sum(mask_dc) > 0 :
        mask_dc_val = np.logical_not(mask_dc)*1000
        y_collied = mask_dc_val*y_dc + y_dc
        dist = np.sqrt( (obst_x2-state[0])**2 + (y_collied-state[1])**2)
        
        output = np.min(dist)

    
    # segment bc (y1 = CTE)    
    x_bc = ( obst[1] - b_array )/m_array
    mask_bc = (obst[0] <= x_bc) & (x_bc <= (obst[0] + obst[2]) )
    
    if np.sum(mask_bc) > 0 :
        mask_bc_val = np.logical_not(mask_bc)*1000
        x_collied = mask_bc_val*x_bc + x_bc
        dist = np.sqrt( (x_collied-state[0])**2 + (obst[1]-state[1])**2)
        
        output = np.min(dist)

    # segment da (y2 = CTE)    
    obst_y2 = (obst[1] + obst[3])
    x_da = ( obst_y2 - b_array )/m_array
    mask_da = (obst[0] <= x_da) & (x_da <= (obst[0] + obst[2]) )
    
    if np.sum(mask_da) > 0 :
        mask_da_val = np.logical_not(mask_da)*1000
        x_collied = mask_da_val*x_da + x_da
        dist = np.sqrt( (x_collied-state[0])**2 + (obst_y2-state[1])**2)
        
        output = np.min(dist)

    # Distance to the Obst. 
    return output
