...
def animate_scene(route_agent_id, data_lists=[], obst_list=[], target_routes=[[]], cm_flag=False, states=[], obs_unknowns=[], smaller=False):
    ...
    # Scene
    for i, route in enumerate(data_lists) :
        scene_axes.append( axes.plot(route[0], route[1], color= mcolors.CSS4_COLORS[colors_list[i]], label="Route "+str(route_agent_id[i]) ) )
...
def plot_jointed_data(data_jointed, target_routes, obst_list, obs_unknowns, cm_flag=False, smaller=False):
    '''
        cm_flag: False when map is maller than 20x20 
    '''
    route_agent_ids = data_jointed['route_agent_ids']
    state_storage_list = data_jointed['state_storage_list']
    data_lists = data_jointed['data_list']    
    ...
    for i, route in enumerate(data_lists) :
        axes.plot(route[0], route[1], color= mcolors.CSS4_COLORS[colors_list[i]], label="Agent "+str(route_agent_ids[i]) )
        axes.scatter(route[0], route[1], color= mcolors.CSS4_COLORS[colors_list[i]], alpha=0.5, linewidths=0.5)
...
def animate_scene_jointed_data(data_jointed, target_routes, obst_list, obs_unknowns, cm_flag=False, smaller=False):
    '''
    ...
    # Scene
    for i, route in enumerate(data_lists) :
        scene_axes.append( axes.plot(route[0], route[1], color= mcolors.CSS4_COLORS[colors_list[i]], label="Agent "+str(route_agent_ids[i]) ) )
        scene_axes.append( axes.scatter(route[0], route[1], color= mcolors.CSS4_COLORS[colors_list[i]], alpha=0.5, linewidths=0.5) )
