...
for i in range(0, total_iter):
    ...
    for agent_idx, robot in enumerate(robot_list):
        ...
        re_frame_list[agent_idx].check_activation(state, obs_detection_flag, goal_coor)

        start_time = time.time()            

        if  re_frame_list[agent_idx].its_on :                  
            state_frame, sensor_dist_frame = re_frame_list[agent_idx].coor_transformation(state, sensor_dist)
            policy_list_drl[agent_idx].idx = 0
            policy_list_drl[agent_idx].Tr = [[state_frame[0], state_frame[1]], [re_frame_list[agent_idx].xg_sub_frame, re_frame_list[agent_idx].yg_sub_frame]]
...
ploting.plot_scene_drl_frame_list(pso_routes.output_routes_ids, data_lists, obst_original, target_routes, cm_flag=True, states=state_storage, obs_unknowns=obst_list_unkowns_converted, frame_objt=re_frame_list)
