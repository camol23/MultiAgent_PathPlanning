...
total_iter = 800                           # For loop Iteretions
...
# Times
execution_time = np.zeros((total_iter+1, 1))
remapping_time = np.zeros((total_iter+1, 1))
avoidace_time = np.zeros((total_iter+1,  1))
policy_time = np.zeros((total_iter+1, 1))

for i in range(0, total_iter):
    # Start time
    start_time = time.time()
    start_exe_time = time.time()
    print("policy.idx", policy.idx)
    # Read Speed
    ...
    if remap_flag and (re_map_activation==0):
        ...
        remapping_time[i, 0] = time.time() - start_time_remapping
    ...
    # print(vr, vl)
    # print(state)
    policy_time[i, 0] = time.time() - start_time
    ...
    execution_time[i, 0] = time.time() - start_exe_time
    time.sleep(T_wait)
    end_time = time.time()
...
print("execution_time mean",  np.mean(execution_time) )
print("execution_time STD",  np.std(execution_time) )
print("execution_time max",  np.max(execution_time) )
print("execution_time min",  np.min(execution_time) )
print("avoidace_time",  np.mean(avoidace_time) )
print("remapping_time max",  np.max(remapping_time) )
print("policy_time",  np.mean(policy_time) )
print("pso_routes_time",  np.mean(pso_routes_time) )
...
if save_time :
    ...
    remap_persistance_flag = 2 # It's just one time activation
    ...
    file_path = '/home/robot/test_local_numpy_based/logging_data/time_data_exp/' # "./logging_data/time_data_exp/"
