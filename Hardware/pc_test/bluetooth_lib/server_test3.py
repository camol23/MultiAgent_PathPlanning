#!/usr/bin/env python3

import time 
import numpy as np
import math
from messaging import *

from utils_fnc import op_funct, interpreter
from control import model, controllers

# ------------------------------------------------------------------
#       Settings
# ------------------------------------------------------------------
T_wait = 0                                  # time.sleep per cycle
cycle_time = 0                              # time to reach the model_step
Ts = 0                                      # Sample time

# Aux.
stop = 0                                    # Stop motor signal
theta_goal = -math.pi

robot_prams = {
    'x_init' : 0,
    'y_init' : 0,
    'theta_init' : 0,

    'l_width' : 0.105,                      # meters
    'r_width' : 0.056/2,                    # meters
    'Ts' : Ts
}

mf_control_params = {
    'kv' : 0.8,                             # Vel. Gain         (sim = 0.8)       
    'kw' : 5,                               # Angular Vel. Gain (sim = 5)
    'xg' : 0.3,                             # Goal x coor.
    'yg' : 0.0,                             # Goal y coor.
    'l_width' : robot_prams['l_width']
}
# ------------------------------------------------------------------


# Store data
vr_list = []
vl_list = []
v_list = []
w_list = []

x_list = []
y_list = []

# Model
robot = model.model_diff_v1()
robot.initialize(robot_prams)

# Controller
policy = controllers.move_forward(mf_control_params)


# Communication
server = BluetoothMailboxServer()
# mbox = TextMailbox("greeting", server)

# The server must be started before the client!
print("waiting for connection...")
server.wait_for_connection()
print("connected!")

# Prepare the messages box
mbox = TextMailbox("greeting", server)


# In this program, the server waits for the client to send the first message
# and then sends a reply.
time.sleep(1)
mbox.send("PC ready to receive")
print("Now time to read")
print()

mbox.wait()
print(mbox.read())

counter = 70
first = 1
i = 0
while True :    
    # for i in range(0, 350):
    # Start time
    start_time = time.time()

    # Ask for data
    mbox.send("PC")

    # Receive State
    mbox.wait()
    vel_right, vel_left = interpreter.state_interpreter(mbox.read(), vis=True)          # Split data
    vel_right, vel_left = op_funct.deg2m_two(vel_right, vel_left, robot.r_wheel)        # from deg/s to m/s
    if first :
        Ts = 0
        first = 0
    else:
        Ts = time.time() - cycle_time                                                       # compute sample time
        
    robot.Ts = Ts
    cycle_time = time.time()
    robot.step(vel_right, vel_left)                                                     # model step
    distance = op_funct.distance(robot.x, robot.y, 0, 0) # compute distance
    
    # Vis.
    print("Distance = ", distance, robot.x, robot.y, op_funct.radToDeg(robot.theta))
    

    # COMPUTE: Policy
    # if distance >= 1.0 :
    theta_e = abs(theta_goal - robot.theta)
    print("Theta error = ", op_funct.radToDeg(theta_e))
    if theta_e <= 0.032 :
        stop = 1 
    mbox.send(str(-counter)+","+str(counter)+","+str(stop))

    print("Ts = ", Ts)
    # vr, vl = policy.step( [robot.x, robot.y, robot.theta], vis=True)
    # vr, vl = op_funct.m2deg_two(vr, vl, robot.r_wheel)
    # mbox.send(str(vr)+","+str(vl)+","+str(stop))

    # Store
    vr_list.append(vel_right)
    vl_list.append(vel_left)
    v_list.append(robot.v)
    w_list.append(robot.w)
    x_list.append(robot.x)
    y_list.append(robot.y)


    # Wait for ACK
    mbox.wait()             
    print(mbox.read())

    # --- next_ev3 --- #

    # counter += 5
    time.sleep(T_wait)
    end_time = time.time()

    # Vis.
    print("*** Total time = ", end_time-start_time, i)


# Compute Data
vr = np.array(vr_list)
vl = np.array(vl_list)
v = np.array(v_list)
w = np.array(w_list)

x = np.array(x_list)
y = np.array(y_list)


print("Vr  mean = {} - var. = {} ".format(np.mean(vr), np.var(vr)))
print("Vl  mean = {} - var. = {} ".format(np.mean(vl), np.var(vl)))
print("V  mean = {} - var. = {} ".format(np.mean(v), np.var(v)))
print("W  mean = {} - var. = {} ".format(np.mean(w), np.var(w)))

print("x  mean = {} - var. = {} ".format(np.mean(x), np.var(x)))
print("y  mean = {} - var. = {} ".format(np.mean(y), np.var(y)))